...
function initializeAttendance() {
    students.forEach(student => {
        attendance[student.regNo] = true; // true = present
    });
}
...
function renderStudents() {
    ...
    students.forEach((student, index) => {
        ...
        card.className = `student-card ${attendance[student.regNo] ? 'present' : 'absent'}`;
        card.dataset.regNo = student.regNo;
        ...
        card.innerHTML = `
            ...
            <div class="status-toggle">${attendance[student.regNo] ? 'Present' : 'Absent'}</div>
        `;

        card.addEventListener('click', () => toggleAttendance(student.regNo));
...
function updateStudentCard(regNo) {
    const card = document.querySelector(`.student-card[data-reg-no="${regNo}"]`);
    if (card) {
        const isPresent = attendance[regNo];
...
function toggleAttendance(regNo) {
    attendance[regNo] = !attendance[regNo];
    updateStudentCard(regNo);
    updateCounts();
}

function markAllAbsent() {
    students.forEach(student => {
        attendance[student.regNo] = false;
    });
    renderStudents();
    updateCounts();
